# Sobre o projeto - Portal Napne

O projeto Portal NAPNE é pensado tendo como objetivo o atendimento das
demandas do Núcleo de Atendimento para as Pessoas com Necessidades
Educacionais Específicas (NAPNE) do Instituto Federal de Educação, Ciência e
Tecnologia do Rio Grande do Norte (IFRN) campus São Paulo do Potengi (SPP),
principalmente ao que se refere às dificuldades enfrentadas quanto ao envio (por
parte dos professores) e recebimento (por parte do NAPNE) do Plano Educacional
Individualizado (PEI), além da disseminação dos comunicados e das ações do núcleo
para a comunidade escolar.

---

## Funcionalidades

- Cadastro e autenticação de diferentes perfis através da API do SUAP (alunos, professores e equipe do NAPNE);
- Envio do PEI pelos professores e recebimento e retorno pela equipe NAPNE;
- Envio de feedback tanto público quanto privado pelos usuários e recebimento por parte do NAPNE;
- Publicação de notícias pelo NAPNE e visualização delas pelos demais usuários;
- Publicação de notificações pelo NAPNE e visualização delas pelos demais usuários.

---

## Tecnologias Utilizadas

[![SkillIcons](https://skillicons.dev/icons?i=py,html,django,bootstrap)](https://skillicons.dev)

---

## Equipe do Projeto

### **Discentes Desenvolvedores**
- Ibrahim Guilherme Moraes de Oliveira 
- Luanna Davi de Souza

### **Orientadora**
- Prof.ª Fernanda Lígia Rodrigues Lopes
